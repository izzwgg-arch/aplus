...
const links = [
  { to: "/aplus", label: "Overview", icon: "M3 13h8V3H3zm10 8h8V11h-8zM3 21h8v-6H3zm10-10h8V3h-8z", permission: "aplus.dashboard.view" },
  {
    external: true,
  ...
  { to: "/aplus/clients", label: "Clients", icon: "M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M8.5 11A4 4 0 1 0 8.5 3a4 4 0 0 0 0 8m8 1a3 3 0 1 0 0-6m4 15v-2a4 4 0 0 0-3-3.87", permission: "aplus.clients.view" },
  { to: "/aplus/services", label: "Services", icon: "M4 6h16M4 12h16M4 18h16M7 6v12M17 6v12", permission: "aplus.services.view" },
  { to: "/aplus/providers", label: "Providers", icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11A4 4 0 1 0 9 3a4 4 0 0 0 0 8m12 10v-2a4 4 0 0 0-3-3.87m-2-8.13a4 4 0 1 1 0-8", permission: "aplus.providers.view" },
  { to: "/aplus/appointments", label: "Appointments", icon: "M8 2v4M16 2v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2", permission: "aplus.appointments.view" },
  { to: "/aplus/reminders", label: "Reminders", icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z", permission: "aplus.communications.view" },
  { to: "/aplus/waitlist", label: "Waitlist", icon: "M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01", permission: "aplus.waitlist.view" },
  { to: "/aplus/invoices", label: "Invoicing", icon: "M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M8 13h8M8 17h8M8 9h2", permission: "aplus.billing.view_invoices" },
  { to: "/aplus/payments", label: "Payments", icon: "M2 7h20M2 12h20M2 17h20M6 4v16M18 4v16", permission: "aplus.billing.view_payment_history" },
  { to: "/aplus/intake", label: "Intake Form", icon: "M4 4h16v16H4zM8 8h8M8 12h8M8 16h5", permission: "aplus.intake.view" },
  { to: "/aplus/settings", label: "Settings", icon: "M12 15.5A3.5 3.5 0 1 0 12 8.5a3.5 3.5 0 0 0 0 7zM19.4 15a1.7 1.7 0 0 0 .34 1.87l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.7 1.7 0 0 0-1.87-.34 1.7 1.7 0 0 0-1.03 1.55V21a2 2 0 1 1-4 0v-.09a1.7 1.7 0 0 0-1.03-1.55 1.7 1.7 0 0 0-1.87.34l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.7 1.7 0 0 0 4.6 15a1.7 1.7 0 0 0-1.55-1.03H3a2 2 0 1 1 0-4h.09A1.7 1.7 0 0 0 4.64 8a1.7 1.7 0 0 0-.34-1.87l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.7 1.7 0 0 0 9 3.64 1.7 1.7 0 0 0 10.03 2.1V2a2 2 0 1 1 4 0v.09A1.7 1.7 0 0 0 15.06 3.64a1.7 1.7 0 0 0 1.87-.34l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.7 1.7 0 0 0 19.4 8a1.7 1.7 0 0 0 1.55 1.03H21a2 2 0 1 1 0 4h-.09A1.7 1.7 0 0 0 19.4 15z", permission: "aplus.settings.view" },
  { to: "/aplus/users", label: "Users", icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11A4 4 0 1 0 9 3a4 4 0 0 0 0 8m12 10v-2a4 4 0 0 0-3-3.87m-2-8.13a4 4 0 1 1 0-8", permission: "aplus.users.view" },
  { to: "/aplus/audit-logs", label: "Audit Logs", icon: "M9 17v-6M13 17V7M17 17v-3M4 19h16M6 19V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v14", permission: "aplus.audit_logs.view" }
];

export default function Sidebar({ open, setOpen }) {
  const { logout } = useAuth();
  const { can, isLoading } = usePermissions();
  const visibleLinks = links.filter((item) => item.external || isLoading || can(item.permission));
